...
import { UnoGameService, CardColor, CardType, Player } from './uno-game.service';

@Injectable()
export class UnoBotService implements OnModuleInit {
  private readonly logger = new Logger(UnoBotService.name);
  ...
  private initializeBot() {
    // Используем отдельный токен для UNO бота, чтобы избежать конфликта с другими ботами
    const botToken = this.configService.get<string>('UNO_BOT_TOKEN') || '8336559057:AAHtmyn93GYhjHQl9vczBem_NxnU4epaM0U';
    
    if (!botToken) {
      this.logger.warn('UNO_BOT_TOKEN not configured, UNO bot disabled');
      return;
    }
    ...
    try {
      ...
      this.setupCommands();
      this.logger.log(`UNO Telegram bot initialized with token: ${botToken.substring(0, 10)}...`);
      
      // Проверяем, что бот работает
      this.bot.getMe().then((botInfo: any) => {
        this.logger.log(`UNO Bot is ready! Username: @${botInfo.username}`);
      }).catch((error: any) => {
        this.logger.error('Failed to get bot info', error);
      });
    } catch (error) {
      this.logger.error('Failed to initialize UNO Telegram bot', error);
    }
  }
  ...
  private setupCommands() {
    ...
    this.bot.onText(/\/uno_start/, async (msg: Message) => {
      try {
        ...
        this.logger.error('Error handling /uno_start', error);
      }
    });
    ...
    this.bot.onText(/\/uno_join/, async (msg: Message) => {
      try {
        ...
        this.logger.error('Error handling /uno_join', error);
      }
    });
    ...
    this.bot.onText(/\/uno_go/, async (msg: Message) => {
      try {
        ...
        this.logger.error('Error handling /uno_go', error);
      }
    });
    ...
    this.bot.onText(/\/uno_leave/, async (msg: Message) => {
      try {
        ...
        this.logger.error('Error handling /uno_leave', error);
      }
    });
    ...
    // Используем более специфичный обработчик, чтобы не конфликтовать с командами
  ...
  private async handlePrivateMessage(msg: Message) {
    ...
    const player = userGame.game.players.find((p) => p.userId === userId);
    if (!player) return;

    const wildCard = player.cards.find((c) => c.type === CardType.WILD || c.type === CardType.WILD_DRAW_FOUR);
    if (!wildCard) return;
  ...
  private async sendPlayerCards(game: any, player: Player): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to send cards to user ${player.userId}`, error);

...
export class UnoGameService {
  private readonly logger = new Logger(UnoGameService.name);
  ...
  createGame(chatId: number): string {
    ...
    this.logger.log(`Created game ${gameId} in chat ${chatId}`);
    return gameId;
  }
  ...
  addPlayer(gameId: string, userId: number, username: string, firstName: string): boolean {
    ...
    this.logger.log(`Player ${username} added to game ${gameId}`);
    return true;
  }
